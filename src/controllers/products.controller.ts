...
const getByCategory = async (req : Request, res : Response) => {
    ...
    try {
        const filteredProducts = await Product.find({item_category: itemCategory});
        if (! filteredProducts) {
...
const create = async (req : Request, res : Response) => {
    ...
    try {
        ...
        if (existedBook) {
            return res.status(StatusCodes.BAD_REQUEST).json({errorMsg: "book is already existed"});
        } else {
            const book = new Book({
                ...req.body,
                createdBy: email
...
const update = async (req : Request, res : Response) => {
    ...
    try {
        ...
        if (existedBook && id !== req.body.id.toString()) {
            return res.status(StatusCodes.BAD_REQUEST).json({errorMsg: "Same ID is already existed"});
        }
